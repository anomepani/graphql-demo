...
function parseMetadata(metadata) {
    ...
    //Added copy of site entitySet and store as searchStite
    entitySets.searchSite = entitySets.sites;
...
function generateQueryDefs(entitySets, singletons) {
    ...
    //Added Extra GraphQl for searchSite and added support for search query
    let entitySetSubDefinitions = Object.keys(entitySets).map((key) => key === "searchSite" ? (' ' + key + '(search: String): ' + entitySets[key] + ' ') : ' ' + key + '(id: ID): ' + entitySets[key] + ' ');
...
function generateQueryDefResolvers(entitySets, singletons) {
    ...
    let subDefinitions = Object.keys(defs).map((key) =>
        // Handle generated query for 'searchSite' and preare dynamic condition
        ` ${key} (obj, args, context){ 
        const name = \'${key==="searchSite"?"sites":key}\';
